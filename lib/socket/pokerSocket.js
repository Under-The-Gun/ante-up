...
shuffle(exampleDeck);
const { firstHandPrompt, playerOutOfTurnPrompt } = require('../../client/tablePrompts');
...
module.exports = (io, socket) => {
  ...
  socket.on('login', async(user) => {
    try {
      await User
        .authorize(user)
        .then(user => {
          gameState.users.push(socket.id);
  ...
  socket.on('get-user-count', () => {
    console.log(gameState.users.length);
    if(gameState.users.length === 1) {
      socket.emit('dealer-options');
    } else socket.emit('out-of-turn-options');
  });
  
  socket.on('your-hold-cards', () => {
    gameState.users.forEach(socketId => {
    ...
    const boardCards = { cardOne: exampleDeck.pop(), cardTwo: exampleDeck.pop(), cardThree: exampleDeck.pop(), cardFour: exampleDeck.pop(), cardFive: exampleDeck.pop() };
